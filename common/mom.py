...
logger = logging.getLogger("RabbitMQProcessor")
...
class RabbitMQProcessor:
    ...
    def connect(self):
        ...
        try:
            logger.info(f"Conectando a RabbitMQ en {self.rabbitmq_host}")
            ...
            logger.error(f"Error inesperado durante la conexión a RabbitMQ: {e}")
            return False
    ...
    def publish(self, queue, message, msg_type=None):
        ...
        logger.info(f"Mensaje enviado a la cola: {queue}, tipo: {msg_type}")
